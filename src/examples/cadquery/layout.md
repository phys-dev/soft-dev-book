...
Все промежуточные файлы складываем в отдельную папку и чистим её перед каждым запуском. Мелочь, но она отделяет свежий результат от вчерашнего: если ELEGANT споткнётся, мы увидим пустую папку, а не старые данные, принятые за новые.
...
Начинается всё с описания магнитной структуры. Файл `machine.lte` — это то, с чем физик-ускорительщик работает каждый день: сначала перечислены элементы с их параметрами (длина, сила квадруполя `K1`, угол поворота `ANGLE`), потом из элементов собраны участки, а из участков — кольцо целиком.
...
Прочитать этот файл и посчитать по нему геометрию мы не будем: это уже умеет ELEGANT. Восклицательный знак в начале строки — способ Jupyter вызвать внешнюю программу и забрать её вывод в переменную. Печатаем последние двадцать строк, чтобы убедиться, что счёт дошёл до конца.
...
Теперь читаем то, ради чего всё затевалось, — файл с координатами элементов на плане зала:
...
Посмотрим на первые строки. В таблице по строке на элемент: `s` — путь вдоль орбиты, `X`, `Y`, `Z` — координаты элемента в зале, `theta` — угол поворота в горизонтальной плоскости, `ds` — длина элемента. Дальше нам понадобятся ровно эти колонки плюс тип элемента.
...
А по последним строкам видно, что кольцо замкнулось: на длине 27.43 метра координаты вернулись в ноль с точностью до десятков микрон.
...
Прежде чем строить тела, стоит договориться о том, что означают углы. Форма поворотного магнита задаётся тремя числами: полным углом поворота и двумя углами наклона торцов, `e1` и `e2`. Определены они так же, как в руководстве по MAD, — оттуда и картинки ниже (https://mad8.web.cern.ch/doc/mad8_user.pdf).
...
Проверим конструктор на одном магните: поворот на 22.5 градуса, длина метр, торцы без наклона. Смотрим на него дважды — сначала в проекции по умолчанию, затем сверху, задав направление взгляда `(0, 1, 0)`: так видно и дугу, и сечение.
...
Остальные элементы устроены проще — параллелепипеды и цилиндры, из которых собираются квадруполи, секступоли и вакуумная камера.
...
Труба вакуумной камеры и резонаторы — просто цилиндры:
...
Квадруполь и секступоль — те же параллелепипеды, отличаются только шириной, по которой их потом можно различить на чертеже:
...
Вот теперь главный цикл — тот, ради которого всё писалось. Он идёт по строкам таблицы ELEGANT и для каждой строит тело нужного типа, красит его в свой цвет и ставит в точку `(X, Y, Z)`, повернув на угол `theta`. Поворотные магниты зелёные, квадруполи оранжевые, секступоли синие, вакуумная камера серая, резонаторы красные.

Обрати внимание на две вещи. Во-первых, длина элемента берётся не из описания структуры, а из колонки `ds` — то есть из того, что ELEGANT реально посчитал. Во-вторых, углы наклона торцов магнита запрашиваются отдельным вызовом `element_par` из файла параметров: в таблице координат их нет.
...
Печать в цикле — не украшение, а способ проверки: список элементов с координатами легко пробежать глазами и заметить, что что-то встало не туда.

Сборка готова, но сама по себе она — просто объект в памяти:
...
Чтобы её увидеть, нужно спроецировать сборку на плоскость. Сначала вид сверху — направление проекции `(0, 1, 0)`, то есть взгляд вертикально вниз, обычный план зала:
...
А теперь изометрия — взгляд из точки `(1, 1, 1)`:
...
Дальше модель выгружается в форматы, которые понимают инженерные пакеты. STEP — трёхмерная геометрия целиком:
...
а DXF — плоский чертёж, тот самый план зала, который пойдёт на бумагу:
