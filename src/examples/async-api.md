...
Этот пример стоит особняком среди остальных: здесь нет ни пучков, ни разностных схем. Зато он показывает архитектуру типичного современного веб-сервиса; именно так сегодня устроены системы управления установками, электронные журналы экспериментов и порталы доступа к данным. Разобравшись с этим примером, ты сможешь написать сервис, который отдаёт данные твоего эксперимента по HTTP.

Задача сервиса проста: по идентификатору фильма вернуть информацию о нём. Данные лежат в полнотекстовом поисковом движке Elasticsearch, а часто запрашиваемые ответы кешируются в Redis, чтобы не дёргать поиск лишний раз. Всё общение с внешними хранилищами — асинхронное: пока один запрос ждёт ответа от базы, сервис обслуживает десятки других. Механику `async`/`await`, лежащую в основе, мы разбирали в главе про [асинхронность](../perf/async.md).

## Структура проекта

Проект разбит на слои, и это главное, на что стоит смотреть в примере. Каждый каталог отвечает ровно за одну задачу:
...
Все настройки читаются из переменных окружения со значениями по умолчанию для локальной разработки. Это [12-факторный подход](https://12factor.net/config): один и тот же образ приложения работает и на ноутбуке, и в продакшене — меняются только переменные окружения, а не код.
...
Обрати внимание, что моделей на самом деле две: `models.Film` с полным набором полей и `Film` из слоя API всего с двумя. Это не дублирование, а сознательное разделение: первая описывает то, что лежит в хранилище, вторая — то, что мы обещали отдавать наружу. Добавив поле в хранилище, ты не сломаешь контракт API случайным образом.
