...
Модель остаётся единственным описанием того, как выглядит фильм, и на неё опираются все слои сразу. Pydantic разбирает по ней ответ Elasticsearch, он же сериализует объект в кеш и обратно. Класс `Config` подменяет стандартный модуль `json` на заметно более быстрый `orjson`, ведь на кешируемых ответах сериализация оказывается не самой дешёвой частью запроса.
