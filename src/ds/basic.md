...
Для физика этот материал ложится на уже знакомый фундамент. Задача обучения модели — задача оптимизации; функция потерь — функционал, который минимизируют; регуляризация — способ сделать некорректную задачу корректной. Всё это ты уже видел, просто под другими названиями.
...
Типы можно и преобразовывать: разбей числовой признак на интервалы — получишь порядковый.

## Что такое модель?
**Модель машинного обучения** — параметрическая функция (или «чёрный ящик»), которая отображает пространство признаков `X` в пространство ответов `y`: `Model: X -> y`.
...
На то, что в итоге получится, влияет всё сразу: качество данных, выбор алгоритма, его гиперпараметры и аккуратность самой процедуры обучения.
