...
### Скрытые слои
Решением является добавление скрытых слоёв. Скрытые слои (\\(Z\\)) формируют новое признаковое пространство, в котором задача решается проще.
...
$$D(w) = \frac{1}{n}$$
Берём случайные значения и умножаем на \\(\frac{1}{\sqrt{n}}\\) (или используем распределение с дисперсией \\(1/n\\)).
