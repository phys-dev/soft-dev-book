...
Когда готовой векторной операции не нашлось, остаётся `apply`. Он прогоняет твою функцию по каждой строке (`axis=1`) или по каждому столбцу. Средство это последнее, потому что внутри работает обычный цикл Python со всеми его издержками, так что перед тем, как писать `apply`, поищи векторное решение ещё раз.
