...
встретится ниже прямо в выводе). Интересна здесь только структура — текстовая
таблица с заголовком, где четыре числовых столбца и один строковый. Сделай такой
файл сам, первая строка — имена столбцов через `, `:
...
Тип массива стоит выбирать сознательно. От него зависит и занимаемая память, и диапазон представимых значений. Посмотри, что происходит с числом 70000, когда его просят уложить в `uint16`, оно молча превращается в 4464. NumPy предупреждает об этом (и в будущих версиях будет считать ошибкой), но само по себе такое переполнение — классический источник тихо неверных результатов.
...
важен: файл с табуляцией, а не с запятой, и `sep='\t'` обязателен.
...
Это и есть знаменитый `SettingWithCopyWarning`. Pandas прямо говорит, что не знает, во что ты сейчас записал — в исходную таблицу или в её копию. Понимать это предупреждение как ошибку и переписывать такое место через `loc` — привычка, которая сэкономит не один вечер отладки.
...
Скользящее окно (rolling) сглаживает шумный ряд. По каждой точке считается среднее (или сумма, или максимум) нескольких соседних. Первые несколько значений оказываются пустыми — окну просто не из чего считать.
...
Обрати внимание, что скользящее окно дружит с группировкой. Тогда оно считается независимо внутри каждой группы и не размазывает соседние группы друг по другу.
...
У строковых колонок есть аксессор `.str`, через который к каждому элементу применяются обычные строковые методы. Их можно выстраивать цепочкой. Ниже из полного имени пассажира вытаскивается личное имя: привести к нижнему регистру, заменить запятую, разрезать по точке и взять вторую часть.
...
Когда готовой векторной операции не нашлось, остаётся `apply`. Он прогоняет твою функцию по каждой строке (`axis=1`) или по каждому столбцу. Средство это последнее — внутри работает обычный цикл Python со всеми его издержками, так что перед тем, как писать `apply`, поищи векторное решение ещё раз.
