...
Мы разобрались, как устроен Python и как писать на нём сопровождаемые программы. Пришло время заняться данными, ради которых физик вообще берёт в руки язык программирования.
...
Решение придумано давно и называется **NumPy**. Идея в том, чтобы хранить числа так, как их хранит C, то есть подряд, одного типа, без обёрток, а операции над ними отдавать скомпилированному коду, работающему сразу над всем массивом. Выигрыш получается в десятки и сотни раз. На нём и стоит весь научный стек: SciPy, pandas, scikit-learn и библиотеки машинного обучения построены на массивах NumPy.
...
Всё держится на одном объекте, который называется [ndarray](https://docs.scipy.org/doc/numpy/reference/generated/numpy.ndarray.html).
...
* [empty](https://docs.scipy.org/doc/numpy/reference/generated/numpy.empty.html) создаёт массив заданной размерности, не инициализированный никаким значением
...
Срез массива в NumPy даёт представление на те же данные, а не их копию. Изменишь срез вроде `x[2:5]`, и изменится исходный массив. А вот отбор по маске или по списку индексов, как в примере выше, всегда возвращает копию, и исходный массив остаётся нетронутым. Когда нужны свои данные, есть метод `copy`.
...
файл сам, а в первой строке помести имена столбцов через `, `:
...
`genfromtxt` с `names=True` возвращает структурированный массив. Строку из него можно запросить по номеру, а колонку по названию.
...
При чтении из файла можно пропускать строки в начале и в конце с помощью **skip_header** и **skip_footer**, а также брать только нужные столбцы через **usecols**.
...
Broadcasting снимает требование одинаковой формы и разрешает арифметику над массивами разных, но согласованных между собой размерностей. Проще всего это видно на умножении вектора на число.
...
Тот же трюк со столбцами так просто не пройдёт, потому что вектор из четырёх элементов согласовать с матрицей по последней оси не удастся.
...
Почти у каждой сводной операции есть аргумент `axis`, задающий номер оси, вдоль которой она считается. Без него операция берёт весь массив целиком и возвращает одно число, а с ним сворачивает только указанную ось, оставляя остальные. На трёхмерном примере это видно нагляднее всего.
...
Тип массива стоит выбирать сознательно. От него зависит и занимаемая память, и диапазон представимых значений. Посмотри, что происходит с числом 70000, когда его просят уложить в `uint16`, оно молча превращается в 4464. NumPy предупреждает об этом (и в будущих версиях будет считать ошибкой), но само по себе такое переполнение остаётся классическим источником тихо неверных результатов.
...
Есть соблазн применять к массиву обычную функцию Python поэлементно. NumPy предлагает для данной цели несколько инструментов, и разница в скорости между ними поучительна.
...
Разница в семьдесят раз выглядит убедительно, но выводы из неё делать рано. `np.vectorize` и генератор списка и правда вызывают `f` сто тысяч раз, и 130–150 миллисекунд составляют цену ста тысяч вызовов функции Python. А `apply_along_axis` с `axis=0` на одномерном массиве вызывает `f` один-единственный раз, передав в неё весь массив целиком, и измерен здесь один векторный `np.sqrt`, а не поэлементный обход.

Мораль та же, о которой говорилось во введении. Прежде чем поверить отношению времён, разберись, что каждая из версий делает. А полезный вывод здесь другой: `np.vectorize` не векторизует, он лишь оборачивает цикл, и ускорения от него ждать не стоит.

## Pandas

[pandas.pydata.org/docs/](https://pandas.pydata.org/docs/)

Pandas читает данные, приводит их в порядок, считает по ним сводки и тут же рисует графики. Если NumPy даёт массив чисел, то pandas даёт таблицу с именованными колонками и осмысленным индексом строк.
...
Скачанный из другого места файл вполне может оказаться обычным CSV.
...
Самая частая ошибка новичка в pandas состоит в том, чтобы не заметить, что срез иногда представление на исходную таблицу, а иногда её копия. Разберём по порядку. Сначала как выбирать строки и столбцы, потом как отличить одно от другого.

### Индексация

Начнём с сортировки и заодно увидим, что индекс строки при этом сохраняется и перестаёт совпадать с её номером по порядку.
...
Это и есть знаменитый `SettingWithCopyWarning`. Pandas прямо говорит, что не знает, во что ты сейчас записал, в исходную таблицу или в её копию. Привычка понимать это предупреждение как ошибку и переписывать такое место через `loc` сэкономит не один вечер отладки.
...
Одномерный срез датафрейма даёт `pd.Series`, колонку значений плюс индекс.

Достать из `pd.Series` обычный `np.array` можно, но обычно не нужно. Вместе с массивом теряется индекс, а он в pandas и есть самое ценное.
...
Средний возраст пассажира по классу каюты можно посчитать в лоб, тремя почти одинаковыми строками. Так делать не надо. Классов может оказаться тридцать, и тридцать почти одинаковых строк ты писать не станешь. Для этого и есть `groupby`.
...
Время в данных эксперимента заслуживает отдельного разговора. Показания приборов почти всегда приходят с меткой времени. Добавим в таблицу колонку с временем в формате Unix и посмотрим, что pandas умеет делать с ней после преобразования к своему типу.
...
Скользящее окно (rolling) сглаживает шумный ряд. По каждой точке считается среднее (или сумма, или максимум) нескольких соседних. Первые несколько значений оказываются пустыми, потому что окну не из чего считать.
...
У строковых колонок есть аксессор `.str`, через который к каждому элементу применяются обычные строковые методы. Их можно выстраивать цепочкой. Ниже из полного имени пассажира вытаскивается личное имя. Строку приводят к нижнему регистру, заменяют запятую, разрезают по точке и берут вторую часть.
...
В реальных данных дырки есть всегда. Прибор не сработал, поле анкеты осталось пустым. Pandas обозначает такие места как `NaN` и даёт несколько способов с ними обойтись. Можно выбросить строки, заполнить их значением, найти их маской.
...
Когда готовой векторной операции не нашлось, остаётся `apply`. Он прогоняет твою функцию по каждой строке (`axis=1`) или по каждому столбцу. Средство это последнее, потому что внутри работает обычный цикл Python со всеми его издержками, и перед тем, как писать `apply`, поищи векторное решение ещё раз.
