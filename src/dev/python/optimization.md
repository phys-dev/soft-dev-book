...
Python — очень гибкий язык, и ровно эта гибкость закрывает дорогу многим оптимизациям. Эффективная оптимизация всегда опирается на предположения и ограничения: чем меньше компилятору позволено считать заранее известным, тем меньше у него простора. Разберём три главные причины, по которым Python платит за удобство скоростью.

## 1. Динамическая типизация

Тип значения становится известен только во время исполнения, и отсюда две беды. Во-первых, интерпретатор проверяет типы на каждом шаге, а это время. Во-вторых, он не знает заранее, с чем работает, и потому обязан честно исполнять весь код: выбросить заведомо ненужную ветку или посчитать выражение заранее он не вправе.
...
Отсюда следствие: интерпретатор обязан честно исполнять то, что написано. Он не может вынести проверку `i == 0` из цикла, потому что не знает, не поменяется ли `a`, `i` или сам `range` по дороге. Такую оптимизацию приходится делать руками.
...
Гадать, какая часть программы работает дольше всего, бесполезно: интуиция врёт систематически. Профилирование отвечает на этот вопрос точно.
...
Иногда полноценный профиль не нужен: надо просто замерить время одной конкретной функции. Для этого в стандартной библиотеке есть модуль `timeit`.
...
Дальше речь пойдёт об уровнях 3–5. Это не значит, что первые два менее важны: как раз у них потенциал ускорения наибольший, и цена ошибки тоже, потому что переделывать архитектуру посреди работы дорого.
...
Займёмся третьим уровнем — самой реализацией. Ниже дюжина приёмов, и каждый с замером. Обрати внимание: не все они дают выигрыш, и это тоже часть урока.
...
Пример игрушечный, но мораль общая: если нужное уже есть в `builtins`, почти всегда быстрее взять готовое, чем писать своё. Встроенные функции реализованы на C, и цикла на уровне интерпретатора в них нет.
...
Мораль: `builtins` и генераторы не ускоряют код автоматически. Стоило заменить `lambda` на готовый метод `str.isdigit`, и картина стала обратной, `filter` выиграл. Проверяй свой случай замером.
...
Мораль: сортировка на месте заметно быстрее, потому что не копирует список. Если исходный порядок не нужен, пользуйся ей.
...
Мораль: самый быстрый вариант здесь заодно и самый идиоматичный — `if not elements`. Так бывает нечасто, но приятно.
...
Мораль: исключение дёшево бросить один раз и дорого — миллион. Выбирай между `hasattr` и `try/except` по тому, какая ситуация встречается чаще. Тот же принцип работает везде, где есть выбор между «спросить разрешения» и «обработать последствия», — например, при запросе по сети.
...
Не пиши численные расчёты циклами на Python. Для этого есть библиотеки на C и Фортране, и разница получается не в проценты, а в десятки раз:
...
Дальше идут приёмы, которые ухудшают читаемость кода ради нескольких процентов. Применяй их, только если профилировщик показал, что эти проценты действительно нужны.
...
Поиск атрибута в Python — операция не бесплатная: за ней стоит `__getattribute__`, а если тот ничего не нашёл, то и `__getattr__`. Напрашивается приём: найти атрибут один раз и сохранить в локальную переменную, чтобы не искать заново на каждой итерации.
...
И вот результат: приём, который годами кочует по советам об оптимизации, здесь проигрывает. Начиная с версии 3.11 CPython специализирует вызов метода прямо в байт-коде, и обычный `result.append(item)` оказывается быстрее заранее сохранённой ссылки. Отличная иллюстрация к главному правилу: любой совет об оптимизации проверяй замером на своей версии интерпретатора.
...
Список объектов `SomeClass` занимает ровно столько же, сколько список целых чисел. Ничего загадочного: `sys.getsizeof` меряет размер самого списка, то есть массива указателей, а не то, на что эти указатели ведут. Надёжно он работает только для простых типов и встроенных структур.
...
А вот долгоживущие бесполезные объекты получить легко, и на практике именно это обычно и называют утечкой. Три классических способа — изменяемый аргумент по умолчанию, забытая переменная, которая живёт всё время работы длинной функции, и кеш в атрибуте класса, из которого ничего никогда не удаляется:
...
А вот с наследованием со `__slots__` неудобно: указывать его приходится в каждом классе иерархии, иначе `__dict__` вернётся и вся экономия пропадёт.
...
Тот же приём — считать вместо того, чтобы хранить, — работает и для более сложных последовательностей. Промежуточный вариант: считать, но кешировать то, что запрашивают чаще всего.
...
* Для памяти работают свои приёмы: `__slots__`, `array`, генераторы вместо списков — они не ускоряют код, но позволяют обработать данные, которые иначе не поместились бы.
