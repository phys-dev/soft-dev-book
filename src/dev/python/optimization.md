...
В первую очередь мы пишем работающий код, а потом быстрый. Мы будем заниматься оптимизацией, когда функционал будет готов.

**Следствие:**

Производительность будет одна и та же все время, пока кто-то не найдёт легко исправимые вещи, которые сделаю программу быстрее без необходимости переделки большого количества кода.

Может повезти, а может не повезти.

**Правильный путь:**

Если тебе нужна быстрая программа — сразу обращай внимание на производительность. <br>
Твой прототип должен быть быстрый — может даже быстрее, чем финальная версия.

Лучше начать с производительного решения и поддерживать его, чем надеяться, что получится оптимизировать медленное решение.
...
Возможно, ты оптимизируешь какую-то функцию в 10 раз. <br>
...
A: Сам код `timeit` сделан в виде <a href="https://github.com/python/cpython/blob/master/Lib/timeit.py#L69">шаблонной строки</a>, куда подставляются параметры. <br>
...
То как система работает. Какие данные обрабатываются, как обрабатываются, объём данных, хранение и т.д.
...
1. На оптимизацию тратится время. Кроме того, не факт, что твои оптимизации что-то дадут
...
Однако в большинстве случаев вместо того, чтобы писать что-то своё лучше использовать готовую функцию из `builtins.`

### Совет 2. Правильная фильтрация

Попробуем получить новый список, отфильтровав только нечетные элементы. <br>
...
Давай напишем код, который лучше отражает наши намерения и будет сразу создавать нужный список:
...
Стоит проверять конкретно твой случай.
...
Предположим, мы хотим написать код, который будет обрабатывать объекты как имеющие некоторый атрибут, так и нет.
...
Разница станет еще большей, если нужно будет проверять несколько атрибутов.
...
Мораль: думай какая ситуация чаще встречается и исходя из этого выбирай из двух вариантов. <br>
...
Используй готовые библиотеки, которые написаны на C и Fortran
...
Используй советы ниже только если это действительно даст какой-то существенный выигрыш
...
В Python поиск атрибута сложная операция. Вызывается `__getattr__` и `__getattribute__`. <br>
Можно найти атрибут один раз и сохранить его, чтобы не искать повторно:
...
`array` позволяет более компактно хранить объекты примитивных типов.
...
`np.array` так же хранит объекты определённых типов и занимает меньше места, чем стандартный `list`.
...
Использование `__slots__` позволяет заметно сократить объём занимаемой памяти:
...
Кроме того у `__slots__` есть дополнительный плюс — ускорение времени обращения к атрибуту
...
* Оптимизация всегда что-то стоит: время разработки, читаемость кода, иногда корректность. Прежде чем ускорять, стоит убедиться, что медленно — это действительно проблема.
