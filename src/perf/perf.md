...
Задача — умножение матриц. Операция самая обычная, встречается в любом физическом расчёте, и при этом на ней видны все типичные способы оптимизации.
...
Итого умножение двух матриц 64×64 занимает 87 миллисекунд, почти десятую долю секунды. Почему так медленно?
...
Операция `list.__getitem__`, как видно, не бесплатна. Поменяем местами циклы `for` так, чтобы код делал меньше обращений по индексу.
...
Со встроенными списками Python компилятор Numba работать толком не умеет — чтобы породить машинный код, ему нужен массив известного типа. Перепишем `matrix_product` через ndarray.
...
Умножение матриц — учебный пример; посмотрим, что Numba даёт на реальной вычислительной задаче. В коде REDPIC, о котором рассказано в [отдельной главе](../examples/redpic.md), пучок представлен ансамблем макрочастиц, и самая дорогая функция расчёта — суммирование кулоновских сил, которые частицы действуют друг на друга. Это и есть эффект пространственного заряда, из-за которого сильноточный пучок расталкивает сам себя:

$$
\vec{F}_i = \sum_{j \ne i} \frac{\vec{r}_i - \vec{r}_j}{|\vec{r}_i - \vec{r}_j|^3}.
$$

Каждая частица взаимодействует с каждой, поэтому вычислительная сложность — \\(O(N^2)\\), где \\(N\\) — число частиц. Никакой алгоритмической хитрости здесь нет, зато цикл идеально ложится и на JIT-компиляцию, и на распараллеливание, ведь слагаемые для разных \\(i\\) считаются независимо.
...
Чтобы получить параллельную версию, достаточно заменить внешний `range` на `prange` — так Numba узнаёт, какой цикл можно раздать по ядрам:
...
Форма записи здесь непривычная: `njit(func)` вместо `@njit` над определением. Так удобнее, когда одну и ту же функцию нужно померить в нескольких режимах — исходник остаётся один, обёрток несколько.
...
Квадратичность видна прямо в числах. При каждом удвоении \\(N\\) время растёт вчетверо, что и означает \\(O(N^2)\\) без всяких асимптотических рассуждений. JIT-компиляция сложность не меняет: она сокращает время в сто с лишним раз, но кривая остаётся квадратичной. Ускорить константы и улучшить асимптотику — вещи разные.

Дальше, один декоратор даёт больше сотни раз. Это не опечатка — примерно настолько интерпретатор Python проигрывает машинному коду на арифметике в тесном цикле. Причём переписывать не пришлось ни строчки — функция как была на Python, так и осталась.

А вот параллелизм добавляет ещё в 4–5 раз, но не в 10, хотя ядер как раз десять. Часть из них производительные, часть энергоэффективные; к этому добавляются накладные расходы на раздачу работы, которые на маленьких \\(N\\) съедают почти весь выигрыш — при \\(N = 256\\) параллельная версия обгоняет последовательную лишь вдвое. Линейного масштабирования по числу ядер на практике почти не бывает.
...
Второй путь к машинному коду — Cython. Это тот же Python, но с аннотациями типов; он транслируется в C и компилируется. Здесь придётся поработать руками, зато и контроля больше.
...
Попробуем пойти дальше и отключить проверку выхода за границы массива. Проверку переполнения целых отключать не нужно: в Cython она и так выключена по умолчанию, так что весь выигрыш здесь даёт `boundscheck`. Взамен ошибка в индексе перестанет возбуждать `IndexError` и молча полезет в чужую память, поэтому границы такого цикла надо выверять глазами.
...
Оператор `@` — то же самое матричное умножение, что и `X.dot(Y)`, только записанное короче. Под обеими записями один и тот же вызов, и замеры это подтверждают.

Приведение к `float64` в первой строке — не косметика. `np.random.randint` возвращает `int64`, а BLAS умеет только `float32`, `float64` и комплексные типы; на целых матрицах NumPy считает своим циклом и выдаёт около 70 мкс вместо 2.7. Тип данных здесь решает больше, чем выбор между `dot` и `@`.

Взгляни на масштаб. Наивная реализация на чистом Python считала произведение матриц 64×64 около 0.1 секунды, NumPy справляется за единицы микросекунд — ускорение в десятки тысяч раз, и без единой строчки на C с твоей стороны. Под капотом NumPy вызывает BLAS, вылизанную десятилетиями библиотеку линейной алгебры, которая использует векторные инструкции процессора и оптимально работает с кешем.
