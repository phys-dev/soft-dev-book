...
Каждая частица взаимодействует с каждой, поэтому вычислительная сложность — \\(O(N^2)\\), где \\(N\\) — число частиц. Никакой алгоритмической хитрости здесь нет, зато цикл идеально ложится и на JIT-компиляцию, и на распараллеливание: слагаемые для разных \\(i\\) считаются независимо.
...
Чтобы получить параллельную версию, достаточно заменить внешний `range` на `prange` — так Numba узнаёт, какой именно цикл можно раздать по ядрам:
...
Обрати внимание на форму записи: `njit(func)` вместо `@njit` над определением. Так удобнее, когда одну и ту же функцию нужно померить в нескольких режимах — исходник остаётся один, обёрток несколько.
...
Из этой таблицы стоит вычитать три вещи.

Квадратичность видна прямо в числах: при каждом удвоении \\(N\\) время растёт вчетверо, что и означает \\(O(N^2)\\) без всяких асимптотических рассуждений. Заметь, что JIT-компиляция сложность не меняет. Она сокращает время в сто с лишним раз, но кривая остаётся квадратичной; ускорить константы и улучшить асимптотику — вещи разные, и первое не заменяет второго.

Дальше, один декоратор даёт больше сотни раз. Это не опечатка: примерно настолько интерпретатор Python проигрывает машинному коду на арифметике в тесном цикле. Причём переписывать пришлось ровно ноль строк, функция как была на Python, так и осталась.

А вот параллелизм добавляет ещё в 4–5 раз, но не в 10, хотя ядер именно десять. Часть из них производительные, часть энергоэффективные; к этому добавляются накладные расходы на раздачу работы, которые на маленьких \\(N\\) съедают почти весь выигрыш: при \\(N = 256\\) параллельная версия обгоняет последовательную лишь вдвое. Линейного масштабирования по числу ядер на практике почти не бывает, и планировать расчёт нужно исходя из этого.
...
Второй путь к машинному коду — Cython: тот же Python, но с аннотациями типов, который транслируется в C и компилируется. Здесь придётся поработать руками, зато и контроля больше.
