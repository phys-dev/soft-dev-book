...
Три наблюдения, каждое из которых полезно помнить.

**Квадратичность видна в таблице.** При каждом удвоении \\(N\\) время растёт вчетверо — это и есть \\(O(N^2)\\) в чистом виде, без всяких асимптотических рассуждений. Заметь, что JIT-компиляция сложность не меняет: она сокращает время в сто с лишним раз, но кривая остаётся квадратичной. Ускорение констант и улучшение асимптотики — разные вещи, и первое не заменяет второго.

**Один декоратор даёт больше сотни раз.** Это не опечатка: интерпретатор Python на арифметике в тесном цикле проигрывает машинному коду примерно настолько. Причём переписывать пришлось ровно ноль строк — функция как была на Python, так и осталась.

**Параллелизм добавляет ещё в 4–5 раз, но не в 10.** Ядер десять, а выигрыш от `prange` — четырёхкратный поверх JIT. Часть ядер эффективные, часть энергоэффективные; плюс накладные расходы на раздачу работы, которые на маленьких \\(N\\) съедают почти всё (при \\(N = 256\\) параллельная версия быстрее последовательной лишь вдвое). Линейного масштабирования по числу ядер на практике не бывает почти никогда — планируй исходя из этого.

Что здесь важно с точки зрения физики, а не программирования: расчёт с 4096 макрочастицами на чистом Python занимает 14 секунд на один вызов функции — а в моделировании таких вызовов десятки тысяч, по одному на каждый шаг интегрирования. Разница между 14 секундами и 25 миллисекундами — это разница между «расчёт не сходится за ночь» и «расчёт готов к обеду». Именно это и делает возможным численное моделирование динамики пучка на обычном сервере, без суперкомпьютера.
