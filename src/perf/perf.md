...
Задачей послужит умножение матриц. Операция самая обычная, встречается в любом физическом расчёте, и при этом на ней видны все типичные способы оптимизации.
...
Вроде бы всё работает, но скорость пока неизвестна. Проверь её с помощью магической команды `timeit`.
...
Итого умножение двух матриц 64×64 занимает 87 миллисекунд, почти десятую долю секунды. Причину такой медлительности стоит поискать.
...
Со встроенными списками Python компилятор Numba работать толком не умеет, ведь чтобы породить машинный код, ему нужен массив известного типа. Перепишем `matrix_product` через ndarray.
...
Умножение матриц остаётся учебным примером; посмотрим, что Numba даёт на реальной вычислительной задаче. В коде REDPIC, о котором рассказано в [отдельной главе](../examples/redpic.md), пучок представлен ансамблем макрочастиц, а самой дорогой функцией расчёта оказывается суммирование кулоновских сил, которые частицы действуют друг на друга. Это и есть эффект пространственного заряда, из-за которого сильноточный пучок расталкивает сам себя:

$$
\vec{F}_i = \sum_{j \ne i} \frac{\vec{r}_i - \vec{r}_j}{|\vec{r}_i - \vec{r}_j|^3}.
$$

Каждая частица взаимодействует с каждой, поэтому вычислительная сложность составляет \\(O(N^2)\\), где \\(N\\) обозначает число частиц. Никакой алгоритмической хитрости здесь нет, зато цикл идеально ложится и на JIT-компиляцию, и на распараллеливание, ведь слагаемые для разных \\(i\\) считаются независимо.
...
Форма записи здесь непривычная, `njit(func)` вместо `@njit` над определением. Так удобнее, когда одну и ту же функцию нужно померить в нескольких режимах, ведь исходник остаётся один, а обёрток несколько.
...
Дальше, один декоратор даёт больше сотни раз. Это не опечатка, ведь примерно настолько интерпретатор Python проигрывает машинному коду на арифметике в тесном цикле. Причём переписывать не пришлось ни строчки, и функция как была на Python, так и осталась.

А вот параллелизм добавляет ещё в 4–5 раз, но не в 10, хотя ядер как раз десять. Часть из них производительные, часть энергоэффективные; к этому добавляются накладные расходы на раздачу работы, съедающие на маленьких \\(N\\) почти весь выигрыш, так что при \\(N = 256\\) параллельная версия обгоняет последовательную лишь вдвое. Линейного масштабирования по числу ядер на практике почти не бывает.
...
Вторым путём к машинному коду служит Cython. Это тот же Python, но с аннотациями типов; он транслируется в C и компилируется. Здесь придётся поработать руками, зато и контроля больше.
...
Как же так! Стало только хуже, ведь большая часть кода по-прежнему использует вызовы Python. Избавимся от них, аннотировав код типами.
...
Попробуем пойти дальше и отключить проверку выхода за границы массива. Проверку переполнения целых отключать не нужно, поскольку в Cython она и так выключена по умолчанию, так что весь выигрыш здесь даёт `boundscheck`. Взамен ошибка в индексе перестанет возбуждать `IndexError` и молча полезет в чужую память, поэтому границы такого цикла надо выверять глазами.
...
Оператор `@` выполняет то же самое матричное умножение, что и `X.dot(Y)`, только записан короче. Под обеими записями один и тот же вызов, и замеры это подтверждают.
