...
Оператор `@` — то же самое матричное умножение, что и `X.dot(Y)`, только записанное короче. Под обеими записями один и тот же вызов, а расхождение в замерах — разброс между прогонами, а не свойство операторов.

Взгляни на масштаб. Наивная реализация на чистом Python считала произведение матриц 64×64 около 0.1 секунды, NumPy справляется за сотни микросекунд — ускорение в сотни раз, и без единой строчки на C с твоей стороны. Под капотом NumPy вызывает BLAS, вылизанную десятилетиями библиотеку линейной алгебры, которая использует векторные инструкции процессора и оптимально работает с кешем.
