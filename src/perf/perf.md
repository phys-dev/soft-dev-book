...
Разговор об ускорении легко превратить в набор разрозненных советов. Мы пойдём другим путём, взяв одну задачу и разгоняя её шаг за шагом, каждый раз замеряя результат.
...
Вроде бы всё работает, но скорость пока неизвестна, поэтому проверь её с помощью магической команды `timeit`.
...
Оператор `@` выполняет то же самое матричное умножение, что и `X.dot(Y)`, только записан короче, так как под обеими записями один и тот же вызов, и замеры это подтверждают.
