...
Но есть целый класс задач, где процессор вообще ни при чём. Программа, которая скачивает тысячу файлов или опрашивает сотню приборов по сети, почти всё время **ждёт**. Запрос ушёл, ответ не пришёл, делать нечего. Держать под каждое такое ожидание отдельный поток расточительно — он стоит памяти и переключений, а занимается бездельем.

Асинхронность решает эту самую задачу. Одному потоку поручают тысячу ожиданий сразу — пока один запрос ждёт ответа, выполняется другой. Механика под этим — генераторы и корутины из главы [«Итераторы, генераторы и корутины»](../dev/python/async.md). Функция, умеющая приостановиться и продолжить с того же места, планировщику и нужна.
...
Классический ответ на I/O bound-нагрузку — добавить ресурсов. Но «залить всё железом» может далеко не каждый. Докупать серверы вместо того, чтобы разбираться с кодом, — роскошь для компаний с очень большими бюджетами. В лаборатории этот путь закрыт наглухо, и остаётся второй: писать эффективнее.
...
Код совершенно линейный, и пока запрос один, всё хорошо. Но стоит такому коду обслуживать многих клиентов сразу, и время ответа поплывёт. Бо́льшую часть времени интерпретатор не делает ничего полезного — ждёт запроса от клиента, ждёт ответа от внешнего сайта, ждёт записи в базу. А клиенты в это время ждут его.
...
Присмотримся к проблеме внимательнее. Интерпретатор по-прежнему бо́льшую часть времени ничего не делает — он спрашивает у операционной системы, завершилась ли очередная операция ввода-вывода. Процессы и потоки этого не меняют. Простаивать будет каждый из них, зато добавятся накладные расходы на переключение контекста и память под потоки, так что положение может даже ухудшиться.
...
Код значительно разросся, но ничего страшного не произошло. Разберём изменения по порядку. Перед стартом цикла событий планировщик заводит себе одну особую, бесконечную задачу — `io_task`. Внутри у неё вечный цикл. Он забирает у селектора накопившиеся события и тут же отдаёт управление обратно планировщику.
...
Если из селектора пришли новые события, то обрабатываем их и убираем из обработки файловые дескрипторы. Важный момент — хранение данных о задачах в селекторе. Одна и та же задача может ожидать чтения данных и пытаться записать новые данные. Поэтому в поле `data` и хранится кортеж `(reader, writer)`.
...
Такое поведение возможно благодаря классу `Future` — он прячет колбэки и делает код линейным. Создавать `Future` руками в современном коде почти не приходится, за тебя это делают `create_task` и `gather`.
