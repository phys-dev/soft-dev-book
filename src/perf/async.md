...
Но есть целый класс задач, где процессор вообще ни при чём, ведь программа, скачивающая тысячу файлов или опрашивающая сотню приборов по сети, почти всё время **ждёт**. Запрос ушёл, ответ не пришёл, делать нечего. Держать под каждое такое ожидание отдельный поток расточительно, ведь он стоит памяти и переключений, а занимается бездельем.
...
Присмотримся к проблеме внимательнее. Интерпретатор по-прежнему бо́льшую часть времени ничего не делает, а лишь спрашивает у операционной системы, завершилась ли очередная операция ввода-вывода. Процессы и потоки этого не меняют. Простаивать будет каждый из них, зато добавятся накладные расходы на переключение контекста и память под потоки, так что положение может даже ухудшиться.
...
Сама по себе задача представляет собой обёртку над корутиной. У каждой задачи есть свой `id`, учитываемый в планировщике в словаре `task_map`. На его заполненность смотрит планировщик при выполнении задач.
...
`event_loop` должен предоставлять интерфейс для работы с сокетами. Таких методов всего четыре:
- `wait_for_read`,
- `wait_for_write`,
- `_remove_reader`,
- `_remove_writer`.
...
Асинхронность служит не универсальным ускорителем, а инструментом для задач, где программа ждёт. Для расчётов она бесполезна. Одна корутина, занявшая процессор надолго, остановит весь цикл событий: поток по-прежнему один.
