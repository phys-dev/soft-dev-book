...
Предыдущие главы раздела упирались в один и тот же потолок, поставленный глобальной блокировкой: сколько бы потоков ты ни запустил, GIL пропускает через интерпретатор только один из них, а заводить больше одновременно считающих процессов, чем ядер, смысла нет.
