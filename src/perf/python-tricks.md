...
Компиляторы и векторизация будут в следующей главе, а здесь речь о том, что не требует ничего, кроме самого языка. Выигрыш тут обычно скромнее, зато и цена нулевая: код остаётся обычным Python, который прочитает любой коллега.
...
Данный подход тоже не идеален, поскольку смотрит на потребление памяти процессом в отдельные моменты времени, из-за чего учитывает не всё, а результаты заметно плавают от запуска к запуску.
