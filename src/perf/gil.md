...
* Демон — поток, созданный с аргументом `daemon=True`:
...
GIL — мьютекс, гарантирующий, что в каждый момент времени байт-код исполняет только один поток. Появился он не по недосмотру: внутреннее состояние интерпретатора (в первую очередь счётчики ссылок у каждого объекта) не защищено от одновременного доступа, и глобальная блокировка — самый простой способ не дать двум потокам испортить его одновременно. Плата за простоту — ровно то, что мы наблюдали в замере.
...
Раз глобальная блокировка одна на интерпретатор, самый прямой способ её обойти — завести несколько интерпретаторов. Именно это и делают процессы: у каждого свой GIL, и друг другу они не мешают, так что на восьми ядрах восемь процессов честно считают одновременно.
