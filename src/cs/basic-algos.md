...
Разница видна невооружённым глазом. Увеличили массив в 10 раз — линейный поиск замедлился тоже примерно вдесятеро: 11 мкс, 122 мкс, 1.3 мс, 12 мс. А бинарный за те же пять порядков роста стал медленнее всего впятеро, с 0.14 до 0.69 мкс.
...
Но есть и вторая важная характеристика — объём оперативной памяти. Программы, которым её не хватает, зависают и мешают работать остальным, а часто и вовсе не доводят дело до конца.
...
Типичная ситуация — мы сохранили вспомогательную информацию, чтобы тратить меньше времени, то есть «обменяли» память на скорость. Выбор этот бывает непростым: либо хранить больше данных и меньше считать, либо считать медленнее, зато уложиться в память.
...
значений, третье — словарь тех же значений, и оба поэтому \\(O(K)\\).
...
* **Алгоритм** — конечная последовательность точных инструкций; свойства, которые от него требуют, — дискретность, детерминированность, понятность, завершаемость, массовость и результативность.
