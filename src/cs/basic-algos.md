...
Одна из основных задач при работе с алгоритмами состоит в оценке эффективности программы и в поиске наиболее экономичного подхода. Самое простое и поверхностное решение сводится к тому, чтобы написать программу и замерить время её выполнения. Если программа тормозит, её переписывают, оптимизируя решение.
...
И помни, чем чреват перерасход памяти. Программа либо упадёт с ошибкой, либо утонет в файле подкачки.
...
Но для практики он слишком медленный, ведь делители для каждого числа мы перебираем заново. Есть куда более быстрый способ, решето Эратосфена.
...
Если число простое, его наименьшим простым делителем служит оно само. Если число составное, его наименьший простой делитель \\(p\\) уже встречался раньше. Более того, нам встречалось и число \\(i\\), такое, что \\(x = i \cdot p\\). На шаге \\(i\\) мы должны пометить число \\(x\\) как составное и указать его наименьший простой делитель.
