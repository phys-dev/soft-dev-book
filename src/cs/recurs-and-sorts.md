...
Представь, что хочешь найти файл `Kormen.pdf` у себя в папке `C:\books\`. Посмотрим, как это сделать без автоматического поиска. Рассмотрим два варианта.
...
Ошибкой, которую часто допускают, применяя рекурсию, оказывается зацикливание. Обычно это происходит, если забыть указать базовый случай. Другая частая причина ухода в бесконечный цикл состоит в неверном изменении параметров в рекурсивном случае.
...
А вот обратное действие, восстановление по компаратору функции ключа, удаётся не всегда. И, как мы ещё увидим, не все компараторы работают корректно. Поэтому обычно намного проще использовать функцию вычисления ключа сортировки.
...
Условием остановки рекурсии служит массив из одного элемента. Он не нуждается в сортировке, поэтому это базовый случай.
...
А вот устойчивости у неё нет. При разбиении элементы перебрасываются через весь массив, и два равных запросто меняются местами, так что стоявший левее оказывается правее. Поэтому там, где исходный порядок равных элементов важен, берут сортировку слиянием или дописывают в ключ сортировки исходный номер элемента.
