...
Структуры данных мы разобрали — теперь займёмся тем, что с ними делают. Начнём с приёма, который для многих оказывается первым по-настоящему трудным местом в программировании: с рекурсии. А затем применим её к самой изученной задаче в информатике — сортировке.
...
Спасает случайность. Если опору выбирать случайно, никакие входные данные не могут систематически загонять алгоритм в худший случай: плохое разбиение теперь зависит не от массива, а от жребия. Гарантии это по-прежнему не даёт — при очень неудачной череде жребиев быстрая сортировка отработает за \\(O(n^2)\\), но вероятность такого исхода исчезающе мала, и в среднем получается \\(O(n \log n)\\).
...
А вот устойчивости у неё нет. При разбиении элементы перебрасываются через весь массив, и два равных элемента запросто меняются местами: тот, что стоял левее, может оказаться правее. Поэтому там, где исходный порядок равных элементов важен, берут сортировку слиянием — или дописывают в ключ сортировки исходный номер элемента.
