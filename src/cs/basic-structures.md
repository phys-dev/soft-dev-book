...
Выше мы разобрали, как простые и составные типы данных хранятся в оперативной памяти. Теперь перейдём к структурам данных. Структура данных — это способ организации информации в памяти, который позволяет проводить с ней определённый набор операций. Например, быстро искать или изменять данные.
...
Главное, что стоит унести из этой главы: у каждой структуры есть операции, которые она делает быстро, и операции, за которые расплачивается. Универсальной структуры не существует — есть подходящая под задачу. Если в твоём коде элементы постоянно добавляются и снимаются с обоих концов, а ты используешь список, — ты платишь \\(O(n)\\) там, где мог бы платить \\(O(1)\\).
