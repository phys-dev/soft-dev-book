...
В предыдущей главе мы говорили о том, как алгоритм использует процессорное время. Теперь нас будет интересовать потребление оперативной памяти — пространственная сложность алгоритма.
...
Важно понимать, что если ты перестал пользоваться объектом, это ещё не значит, что он освободил занимаемую память. В языке C++ приходится следить за тем, чтобы выделенная память освобождалась. Встроенные контейнеры вроде std::vector задачу упрощают — они сами выделяют память при создании контейнера и освобождают её, когда переменная выходит из области видимости.
...
Вставлять элемент можно в любое место массива, а вставка в начало и в конец — просто частные случаи этой задачи. Вставка в начало — худший случай. Все элементы приходится сдвинуть на одну позицию вправо, а это \\(O(n)\\). Добавление в конец, наоборот, лучший: сдвигать никого не нужно, и стоит оно \\(O(1)\\).
...
Поэтому при расширении массива программе, как правило, приходится произвести «реаллокацию» — выделить новый участок памяти и скопировать туда старое содержимое.

Выделение памяти — долгая операция, но копирование обходится ещё дороже. Пройти придётся по всем элементам, а это \\(O(n)\\). Как же мы тогда можем утверждать, что добавление в конец массива стоит \\(O(1)\\)?
...
Расплата — доступ по индексу. Получить элемент за \\(O(1)\\) уже не выйдет, элементы-то лежат вразброс. До нужного приходится идти от головы списка, шаг за шагом, и это \\(O(n)\\) операций.
...
Со стеком ты уже познакомился и знаешь, как он устроен. Ещё одна важная структура данных — «очередь». В отличие от стека с его LIFO, очередь живёт по принципу FIFO (англ. first in, first out): первым извлекают тот элемент, который добавили раньше всех.
...
Как и стек, очередь — **интерфейс общения с данными.** Он гарантирует набор методов, но ничего не обещает про способ их хранения — «под капотом» может находиться что угодно.
...
Житейская аналогия — колода карт в руках у игрока. Карту можно взять сверху или снизу, подложить сверху или снизу, а вот вытянуть из середины, не разворошив колоду, не получится. Дек ведёт себя точно так же.

Интерфейс дека объединяет стек и очередь:

* `append(item)` — добавить элемент в конец;
* `appendleft(item)` — добавить в начало;
* `pop()` — взять элемент с конца и удалить;
* `popleft()` — взять с начала и удалить.

Отсюда видно, что дек — обобщение обеих разобранных структур. Работаешь только с одним концом — получаешь стек. Кладёшь в один конец, забираешь из другого — получаешь очередь. Потому в Python отдельного класса для очереди и нет; её роль исполняет дек.
...
Список в Python умеет и `insert(0, x)`, и `pop(0)`. Разница в том, во что эти операции обходятся. Список — динамический массив, его элементы лежат в памяти подряд; чтобы вставить элемент в начало, нужно сдвинуть **все** остальные на одну позицию вправо. Это \\(O(n)\\) на каждую вставку. Дек устроен как связанные между собой блоки, и добавление к любому его концу не трогает остальные элементы вовсе.
...
Смотреть надо не на сами числа, а на последний столбец. **Отношение растёт вместе с \\(N\\)** — это и есть подпись разной алгоритмической сложности. Будь дек просто «написан эффективнее», выигрыш держался бы постоянным, скажем всегда в десять раз. А здесь на тысяче элементов разница тринадцатикратная, на ста тысячах — тысячекратная, потому что время списка растёт как \\(O(n^2)\\), а дека — как \\(O(n)\\).
...
А у дека ограничение длины встроено прямо в конструктор — при переполнении он сам выбрасывает элемент с противоположного конца.
...
На потоке из 200 000 отсчётов первый вариант отработал за 21 мс, второй — за 2 мс. Тот самый порядок, о котором шла речь в начале главы. Причём выигран он не оптимизацией кода, а одной строчкой в выборе структуры данных.
...
Туда же попадают переданные в функцию аргументы — с них и начинается набор локальных переменных. В нашем случае есть один аргумент `name = 'Гоша'`, а другие локальные переменные отсутствуют.
...
Разберём один и тот же учебный сервис — список записей с операциями «прочитать страницу», «добавить», «изменить», «удалить» — в четырёх вариантах хранения. Код почти не меняется, а меняется только то, где и как лежат данные, а вместе с этим и сложность операций. Ради этой мысли глава и написана.

### Наивное решение. GET

Самый прямолинейный вариант — обычный текстовый файл, по строке на запись. Чтобы отдать одну страницу, приходится прочитать файл целиком, а при изменении и удалении — ещё и переписать его заново. \\(O(n)\\) на каждую операцию.
...
Если отвести каждой записи одинаковое число байт, файл превращается в массив — нужную запись читаем сразу, перескочив к её смещению через `seek()`. Зато удаление дорожает: хвост файла приходится сдвигать вручную.
...
\\(^{**}\\) В том виде, в каком список описан выше, — с одной только головой: чтобы добавить элемент в конец, приходится пройти весь список. Если хранить ещё и ссылку на хвост, вставка в конец становится \\(O(1)\\) — именно так и устроены реализации в стандартных библиотеках.


У каждой структуры есть операции, которые она делает быстро, и операции, за которые расплачивается. Универсальной не существует — есть подходящая под задачу. Если элементы у тебя постоянно добавляются и снимаются с обоих концов, а в коде стоит список, ты платишь \\(O(n)\\) там, где мог бы платить \\(O(1)\\).
