...
Вставлять элемент можно в любое место массива, а вставка в начало и в конец — просто частные случаи этой задачи. Вставка в начало — худший случай: все элементы приходится сдвинуть на одну позицию вправо, и это \\(O(n)\\). Добавление в конец — лучший случай: сдвигать никого не нужно, и стоит оно \\(O(1)\\).


### Сложность удаления из динамического массива
При удалении, как и при вставке, приходится сдвигать все элементы правее той ячейки, где идёт операция: при добавлении — на один вправо, при удалении — на один влево. Значит, и цена та же: \\(O(n)\\) при работе с началом массива и \\(O(1)\\) при работе с концом.
...
Выделение памяти — долгая операция, но копирование обходится ещё дороже: нужно пройти по всем элементам, а это \\(O(n)\\) операций. Как же мы тогда можем утверждать, что добавление в конец массива стоит \\(O(1)\\)?
...
Расплата — доступ по индексу: получить элемент за \\(O(1)\\) уже не выйдет, ведь элементы лежат в памяти вразброс. Для этого нужно пройти по всем элементам, начиная с головы списка, пока не дойдём до нужного. Это займёт \\(O(n)\\) операций.
...
Житейская аналогия — колода карт в руках у игрока. Карту можно взять сверху или снизу, подложить сверху или снизу, а вот вытянуть из середины, не разворошив колоду, не получится. Дек ведёт себя ровно так же.
...
Отсюда видно, что дек — обобщение обеих разобранных структур. Работаешь только с одним концом — получаешь стек. Кладёшь в один конец, забираешь из другого — получаешь очередь. Поэтому в Python отдельного класса для очереди нет: её роль исполняет дек.
...
Обрати внимание не на сами числа, а на последний столбец: **отношение растёт вместе с \\(N\\)**. Это и есть подпись разной алгоритмической сложности. Если бы дек просто был «написан эффективнее», выигрыш держался бы постоянным — скажем, всегда в десять раз. А здесь на тысяче элементов разница тринадцатикратная, на ста тысячах — тысячекратная, потому что время списка растёт как \\(O(n^2)\\), а дека — как \\(O(n)\\).
...
А на деке ограничение длины встроено прямо в конструктор: при переполнении он сам выбрасывает элемент с противоположного конца.
...
На потоке из 200 000 отсчётов первый вариант отработал за 21 мс, второй — за 2 мс. Ровно тот порядок, о котором шла речь в начале главы, и получен он не оптимизацией кода, а выбором подходящей структуры данных.

За всё приходится платить, и дек платит доступом по индексу. У списка элементы лежат подряд, поэтому `lst[50000]` вычисляется арифметикой по адресу за \\(O(1)\\). У дека до середины приходится добираться по блокам, и это \\(O(n)\\): в том же замере обращение к середине заняло 0.005 мкс у списка против 0.525 мкс у дека — почти в сто раз дольше.

Отсюда простое правило выбора. Нужен произвольный доступ по индексу — бери список. Нужны быстрые операции с обоими концами, а по середине ты только пробегаешь целиком — бери дек. А если ты ловишь себя на `insert(0, x)` или `pop(0)` внутри цикла, считай это сигналом: почти наверняка тебе нужен дек.
...
Туда же попадают переданные в функцию аргументы: с них и начинается набор локальных переменных функции. В нашем случае есть один аргумент `name = 'Гоша'`, а другие локальные переменные отсутствуют.
...
Памяти под стек выделяется совсем немного — несопоставимо меньше, чем всего оперативной памяти в машине. Поэтому рекурсию нельзя загонять глубже нескольких десятков тысяч вызовов: стек переполнится (англ. stack overflow), и программа аварийно завершится. На одних языках это будет исключение «Stack Overflow», на других — ошибка «Segmentation Fault».
...
Разберём один и тот же учебный сервис — список записей с операциями «прочитать страницу», «добавить», «изменить», «удалить» — в четырёх вариантах хранения. Код почти не меняется — меняется только то, где и как лежат данные, а вместе с этим и сложность операций. Это ровно та мысль, ради которой написана глава: способ хранения важнее ловкости кода.
...
Настоящая база данных прячет всю эту механику за индексами: искать по ключу, вставлять и удалять она умеет сама, и делает это не хуже, чем структуры из этой главы.
...
\\(^{**}\\) В том виде, в каком список описан выше, — с одной только головой: чтобы добавить элемент в конец, приходится пройти весь список. Если хранить ещё и ссылку на хвост, вставка в конец становится \\(O(1)\\), и именно так устроены реализации в стандартных библиотеках.
