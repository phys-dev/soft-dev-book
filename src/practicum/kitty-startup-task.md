...
Код в репозитории лежит не в `main`, а в ветках `front_code` и `my_backend` — так его и оставили. Первое, что предстоит сделать, — разобраться, что в них, и свести это вместе.
...
- Наружу опубликован ровно один порт — 80-й у Nginx. У остальных сервисов в `docker-compose.yml` нет секции `ports`, только `expose`.
