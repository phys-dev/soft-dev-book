...
> Опирается на главы [«Инструменты Linux»](../linux/tools.md) и [«Внутреннее устройство Linux»](../linux/structure.md), где разобраны `strace` и `perf`.
...
5. **Работа с perf.**
   ...
   - Учти: в виртуальной машине аппаратные счётчики процессора обычно недоступны, и `perf record` без аргументов упадёт на событии `cycles`. Бери программное событие, например `perf record -e cpu-clock -g …`. Если и оно отказывает, дело в `kernel.perf_event_paranoid`, поэтому посмотри текущее значение через `sysctl` и опусти его настолько, насколько нужно, понимая, что ты разрешаешь
