...
Эту часть книга не покрывает намеренно: рабочее место каждый настраивает под себя, и разбираться надо по документации выбранных инструментов.
...
5. **Работа с perf.**
   ...
   - Учти: в виртуальной машине аппаратные счётчики процессора обычно недоступны, и `perf record` без аргументов упадёт на событии `cycles`. Бери программное событие, например `perf record -e cpu-clock -g …`. Если и оно отказывает, дело в `kernel.perf_event_paranoid`, поэтому посмотри текущее значение через `sysctl` и опусти его настолько, насколько нужно, понимая, что именно ты разрешаешь
...
- Скринкасты в формате asciinema, по одному на каждый пункт практической части. На записи должен быть виден результат, а не только набор команды. У `gtypist` это итоговые Raw speed, Adjusted speed и процент ошибок, у однострочника с `curl` — несколько строк вывода со всеми шестью полями, у мониторинга — ответы на все пять вопросов.
- Название выбранного эмулятора терминала и одна фраза о том, почему выбран он.
- Для задания со звёздочкой нужна ссылка на запись `perf report` и имя самой нагруженной функции с указанием, в каком файле исходников она найдена.
