...
- **Главное: система загружается с одним диском.** Отключи в настройках виртуальной машины первый диск, загрузись со второго, покажи `cat /proc/mdstat`, где массив находится в состоянии `degraded`, а система работает. Потом верни диск и покажи, что массив пересобрался. Без этой демонстрации задание не принимается: RAID1, который не проверяли на отказ, ничем не лучше одного диска.
